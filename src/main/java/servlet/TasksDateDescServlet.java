...
@WebServlet("/tasksDateDesc")
public class TasksDateDescServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            List<Task> taskList = TaskController.getInstance().getTasksByJournalIdDateDesc(journalId);
            request.setAttribute("taskList", taskList);
            getServletContext().getRequestDispatcher("/tasks.jsp").forward(request, response);
        } catch (DaoException e) {
            System.out.println("Error! Class: " + TasksDateDescServlet.class.getName() + ". Date: " +
