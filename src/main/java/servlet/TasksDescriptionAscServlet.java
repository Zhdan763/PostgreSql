...
@WebServlet("/tasksDescriptionAsc")
public class TasksDescriptionAscServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            List<Task> taskList = TaskController.getInstance().getTasksByJournalIdDescriptionAsc(journalId);
            request.setAttribute("taskList", taskList);
            getServletContext().getRequestDispatcher("/tasks.jsp").forward(request, response);
        } catch (ControllerException e) {
            System.out.println("Error! Class: " + TasksDescriptionAscServlet.class.getName() + ". Date: " +
