...
@WebServlet("/tasksStatusDesc")
public class TasksStatusDescServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            List<Task> taskList = TaskController.getInstance().getTasksByJournalIdStatusDesc(journalId);
            request.setAttribute("taskList", taskList);
            getServletContext().getRequestDispatcher("/tasks.jsp").forward(request, response);
        } catch (DaoException e) {
            System.out.println("Error! Class: " + TasksStatusDescServlet.class.getName() + ". Date: " +
